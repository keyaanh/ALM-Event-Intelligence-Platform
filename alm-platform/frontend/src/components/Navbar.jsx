...
const ROLE_LABELS = {
  ...
  vp_outreach: 'VP of Outreach', vp_standards: 'VP of Standards', member: 'Member',
}

// All roles can VIEW all pages — edit is controlled inside each page
const ALL_NAV = [
        ...
        <div style={{ display: 'flex', gap: 1, flex: 1, overflow: 'auto' }}>
          {visibleLinks.map(link => (
            <Link key={link.to} to={link.to} style={{
              padding: '0.4rem 0.85rem', borderRadius: 7, textDecoration: 'none', fontWeight: 600, fontSize: '0.81rem', whiteSpace: 'nowrap',
              color: isActive(link.to) ? 'var(--maroon)' : 'var(--gray-500)',
              background: isActive(link.to) ? 'var(--maroon-faint)' : 'transparent', transition: 'all 0.15s'
            }}>
              {link.label}
            </Link>
          ))}
