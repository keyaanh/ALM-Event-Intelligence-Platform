...
import { Plus, Trash2, CheckSquare, Square, BookOpen, Users, CheckCircle2, Clock, Loader2, Edit3, Save, X } from 'lucide-react'

const SEMESTER = 'Spring 2026'
const STATUS_STYLE = {
  in_progress: { bg:'#fffbeb', color:'#92660a', border:'#fde68a', label:'In Progress' },
  initiated:   { bg:'#f0fdf4', color:'#166534', border:'#86efac', label:'Initiated ✓' },
  dropped:     { bg:'#fef2f2', color:'#991b1b', border:'#fca5a5', label:'Dropped' },
}
const MODULE_COLORS = {
    ...
    <div className="page">
      <div style={{ display:'flex', justifyContent:'space-between', alignItems:'flex-start', marginBottom:'2rem' }}>
        <div>
          ...
          <p style={{ color:'var(--gray-500)', fontSize:'0.84rem', marginTop:3 }}>Pledge tracker, education modules, and member development</p>
        </div>
        {canEdit && (
                    ...
                    <div style={{ display:'flex', alignItems:'flex-start', gap:'1rem' }}>
                      ...
                      <div style={{ flex:1 }}>
                        <div style={{ display:'flex', alignItems:'center', gap:8, marginBottom:3 }}>
                          <span style={{ fontWeight:700, color:'var(--gray-900)', fontSize:'0.95rem' }}>{pledge.full_name}</span>
                        ...
                        <div style={{ display:'flex', gap:10, flexWrap:'wrap' }}>
                          {pledge.phone && <span style={{ fontSize:'0.75rem', color:'var(--gray-500)' }}>📞 {pledge.phone}</span>}
                          {pledge.email && <span style={{ fontSize:'0.75rem', color:'var(--gray-500)' }}>✉️ {pledge.email}</span>}
                          <span style={{ fontSize:'0.75rem', color:'var(--gray-500)' }}>{done}/{reqs.length} requirements</span>
                        </div>
                        {/* Progress bar */}
                        <div style={{ height:4, background:'var(--gray-100)', borderRadius:99, marginTop:6, overflow:'hidden' }}>
                      ...
                      <div style={{ display:'flex', gap:4, flexShrink:0 }}>
                        ...
                        {canEdit && <button onClick={()=>deletePledge(pledge.id)} style={{ background:'none', border:'none', cursor:'pointer', color:'var(--gray-300)', padding:4 }} onMouseEnter={e=>e.currentTarget.style.color='#ef4444'} onMouseLeave={e=>e.currentTarget.style.color='var(--gray-300)'}><Trash2 size={14}/></button>}
                      </div>
                    </div>

                    {isExpanded && (
                      <div style={{ marginTop:'1rem', paddingTop:'1rem', borderTop:'1px solid var(--border)' }}>
                        <p style={{ fontSize:'0.72rem', fontWeight:700, color:'var(--gray-500)', textTransform:'uppercase', letterSpacing:'0.07em', marginBottom:'0.6rem' }}>Requirements Checklist</p>
                        <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:4 }}>
                          {reqs.map(req => (
                            <div key={req.id} onClick={()=>canEdit && toggleRequirement(pledge, req.id)}
                              ...
                              <span style={{ fontSize:'0.79rem', color:req.done?'var(--gray-400)':'var(--gray-700)', textDecoration:req.done?'line-through':'none' }}>{req.task}</span>
                            </div>
                          ))}
                        </div>
                        {pledge.notes && <p style={{ fontSize:'0.8rem', color:'var(--gray-500)', marginTop:'0.75rem', padding:'0.6rem 0.75rem', background:'var(--gray-50)', borderRadius:8 }}>{pledge.notes}</p>}
                      </div>
                    )}
                  </div>
                )
            ...
            <div style={{ display:'grid', gridTemplateColumns:'repeat(auto-fill, minmax(300px,1fr))', gap:'1rem' }}>
              {modules.map(mod => (
                <div key={mod.id} className="card card-hover" onClick={()=>canEdit && toggleModule(mod)}
                  style={{ cursor:canEdit?'pointer':'default', borderLeft:`3px solid ${MODULE_COLORS[mod.module_type]||'var(--maroon)'}`, opacity:mod.completed?0.75:1, background:mod.completed?'var(--gray-50)':'white' }}>
                  <div style={{ display:'flex', justifyContent:'space-between', alignItems:'flex-start', marginBottom:8 }}>
                    ...
                    <div style={{ display:'flex', alignItems:'center', gap:6 }}>
                      ...
                      {canEdit && <button onClick={e=>{e.stopPropagation();deleteModule(mod.id)}} style={{ background:'none', border:'none', cursor:'pointer', color:'var(--gray-300)', padding:2 }} onMouseEnter={e=>e.currentTarget.style.color='#ef4444'} onMouseLeave={e=>e.currentTarget.style.color='var(--gray-300)'}><Trash2 size={13}/></button>}
                    </div>
                  </div>
                  <p style={{ fontWeight:700, color:'var(--gray-900)', marginBottom:4, textDecoration:mod.completed?'line-through':'none' }}>{mod.title}</p>
                  {mod.description && <p style={{ fontSize:'0.8rem', color:'var(--gray-500)', marginBottom:6 }}>{mod.description}</p>}
                  <div style={{ display:'flex', gap:8 }}>
                    {mod.attendee_count > 0 && <span style={{ fontSize:'0.72rem', color:'var(--gray-400)' }}>👥 {mod.attendee_count} attendees</span>}
                    {mod.completed_date && <span style={{ fontSize:'0.72rem', color:'#166534' }}>✓ {new Date(mod.completed_date).toLocaleDateString()}</span>}
                  </div>
                  {canEdit && <p style={{ fontSize:'0.7rem', color:'var(--gray-300)', marginTop:8, fontStyle:'italic' }}>Click to toggle complete</p>}
                </div>
              ))}
            </div>
          )}
