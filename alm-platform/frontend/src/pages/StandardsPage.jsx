...
import { Shield, AlertTriangle, CheckCircle2, XCircle, Clock, Users, TrendingUp, Save, Loader2 } from 'lucide-react'

const STATUS_OPTIONS = ['present','tardy','excused','absent']
const STATUS_STYLE = {
  present:  { bg: '#f0fdf4', color: '#166534', border: '#86efac', label: 'P' },
  tardy:    { bg: '#fffbeb', color: '#92660a', border: '#fde68a', label: 'T' },
  excused:  { bg: '#eff6ff', color: '#1d4ed8', border: '#93c5fd', label: 'E' },
  absent:   { bg: '#fef2f2', color: '#991b1b', border: '#fca5a5', label: 'A' },
  unmarked: { bg: 'var(--gray-50)', color: 'var(--gray-300)', border: 'var(--border)', label: '—' },
}
    ...
    <div className="page">
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '2rem' }}>
        <div>
          ...
          <p style={{ color: 'var(--gray-500)', fontSize: '0.84rem', marginTop: 3 }}>Attendance tracking, strikes, and retreat eligibility</p>
        </div>
        {strikes && (
          <div style={{ display: 'flex', gap: '0.75rem' }}>
            <div className="card" style={{ padding: '0.75rem 1.25rem', textAlign: 'center', minWidth: 100 }}>
              ...
              <div style={{ fontSize: '0.72rem', color: 'var(--gray-500)', fontWeight: 600, textTransform: 'uppercase', letterSpacing: '0.05em' }}>Chapter Rate</div>
            </div>
            <div className="card" style={{ padding: '0.75rem 1.25rem', textAlign: 'center', minWidth: 100 }}>
              <div style={{ fontSize: '1.5rem', fontFamily: 'var(--font-display)', fontWeight: 700, color: '#ef4444' }}>
                {strikes.members?.filter(m => m.retreat_ineligible).length || 0}
              </div>
              <div style={{ fontSize: '0.72rem', color: 'var(--gray-500)', fontWeight: 600, textTransform: 'uppercase', letterSpacing: '0.05em' }}>Ineligible</div>
            </div>
          </div>
        )}
      </div>
          ...
          <div style={{ display: 'flex', gap: '0.5rem', marginBottom: '1.25rem', flexWrap: 'wrap' }}>
            {events.map(e => (
              <button key={e.id} onClick={() => setSelectedEvent(e.id)} style={{
                ...
                color: selectedEvent === e.id ? 'var(--maroon)' : 'var(--gray-700)',
                fontWeight: 600, fontSize: '0.8rem', cursor: 'pointer', transition: 'all 0.15s'
              }}>
                {e.name}
              </button>
            ))}
            {events.length === 0 && <p style={{ color: 'var(--gray-500)', fontSize: '0.84rem' }}>No mandatory events yet. Create events with mandatory types to track attendance.</p>}
          </div>
          ...
          {selectedEvent && (
            <>
              ...
              <div style={{ display: 'flex', gap: '0.75rem', marginBottom: '1rem', flexWrap: 'wrap', alignItems: 'center' }}>
                <span style={{ fontSize: '0.75rem', color: 'var(--gray-500)', fontWeight: 600 }}>Click to cycle:</span>
                {STATUS_OPTIONS.map(s => (
                  <span key={s} style={{ display: 'flex', alignItems: 'center', gap: 4, fontSize: '0.75rem' }}>
                    ...
                    <span style={{ color: 'var(--gray-500)', textTransform: 'capitalize' }}>{s}</span>
                  </span>
                ))}
                {canEdit && (
              ...
              <div className="card" style={{ padding: 0, overflow: 'hidden' }}>
                <div style={{ padding: '0.75rem 1.25rem', borderBottom: '1px solid var(--border)', background: 'var(--gray-50)', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                  <span style={{ fontWeight: 700, fontSize: '0.9rem', color: 'var(--gray-900)' }}>{selectedEventData?.name}</span>
                  ...
                  {members.map((member, i) => {
                    ...
                    return (
                      <div key={member.id} onClick={() => canEdit && cycleStatus(member.id)}
                        ...
                        <span style={{ fontSize: '0.83rem', color: 'var(--gray-900)', fontWeight: 500 }}>{member.full_name}</span>
        ...
        <div className="fade-in">
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4,1fr)', gap: '1rem', marginBottom: '1.5rem' }}>
            {[
              ...
              { label: 'Mandatory Events', value: strikes.total_mandatory_events, color: '#3b82f6' },
              { label: 'Retreat Ineligible', value: strikes.members?.filter(m => m.retreat_ineligible).length, color: '#ef4444' },
            ].map(s => (
            ...
            <table style={{ width: '100%', borderCollapse: 'collapse' }}>
              ...
              <tbody>
                {strikes.members?.map((m, i) => (
                  <tr key={m.member_id} style={{ background: m.retreat_ineligible ? '#fef2f2' : i % 2 === 0 ? 'white' : 'var(--gray-50)', borderBottom: '1px solid var(--border)' }}>
                    <td style={{ padding: '0.65rem 1rem', fontSize: '0.84rem', fontWeight: 600, color: 'var(--gray-900)' }}>{m.full_name}</td>
                    <td style={{ padding: '0.65rem 1rem', textAlign: 'center' }}>
                      <span style={{ fontSize: '0.8rem', color: '#166534', fontWeight: 700 }}>{m.present}</span>
                    </td>
                    <td style={{ padding: '0.65rem 1rem', textAlign: 'center' }}>
                      <span style={{ fontSize: '0.8rem', color: '#92660a', fontWeight: 700 }}>{m.tardy}</span>
                    </td>
                    <td style={{ padding: '0.65rem 1rem', textAlign: 'center' }}>
                      <span style={{ fontSize: '0.8rem', color: '#1d4ed8', fontWeight: 700 }}>{m.excused}</span>
                    </td>
                    <td style={{ padding: '0.65rem 1rem', textAlign: 'center' }}>
                      <span style={{ fontSize: '0.8rem', color: '#991b1b', fontWeight: 700 }}>{m.unexcused}</span>
                    </td>
                    <td style={{ padding: '0.65rem 1rem', textAlign: 'center' }}>
                      <span style={{ display: 'inline-flex', alignItems: 'center', justifyContent: 'center', width: 28, height: 28, borderRadius: '50%', background: m.strikes >= 3 ? '#fef2f2' : m.strikes >= 2 ? '#fffbeb' : '#f0fdf4', color: m.strikes >= 3 ? '#991b1b' : m.strikes >= 2 ? '#92660a' : '#166534', fontWeight: 800, fontSize: '0.85rem', border: `2px solid ${m.strikes >= 3 ? '#fca5a5' : m.strikes >= 2 ? '#fde68a' : '#86efac'}` }}>
                        {m.strikes}
                      </span>
                    </td>
                    <td style={{ padding: '0.65rem 1rem', textAlign: 'center' }}>
                      {m.retreat_ineligible
                        ? <span style={{ display: 'inline-flex', alignItems: 'center', gap: 4, fontSize: '0.72rem', color: '#991b1b', fontWeight: 700, background: '#fef2f2', padding: '0.2rem 0.6rem', borderRadius: 99, border: '1px solid #fca5a5' }}><XCircle size={11} /> Ineligible</span>
                        : <span style={{ display: 'inline-flex', alignItems: 'center', gap: 4, fontSize: '0.72rem', color: '#166534', fontWeight: 700, background: '#f0fdf4', padding: '0.2rem 0.6rem', borderRadius: 99, border: '1px solid #86efac' }}><CheckCircle2 size={11} /> Eligible</span>
                      }
                    </td>
                  </tr>
                ))}
