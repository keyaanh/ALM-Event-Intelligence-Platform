...
import { Megaphone, CheckCircle2, Circle, Plus, ExternalLink } from 'lucide-react'

const SOCIAL_PLATFORMS = [
  { name: '🅾 𝐈𝐧𝐬𝐭𝐚𝐠𝐫𝐚𝐦', url: 'https://instagram.com', color: '#E1306C', bg: '#fdf2f7' },
  { name: 'Canva', url: 'https://canva.com', color: '#00C4CC', bg: '#f0fffe', icon: '🎨' },
  { name: 'LinkedIn', url: 'https://linkedin.com', color: '#0A66C2', bg: '#f0f6ff', icon: '💼' },
  { name: 'GroupMe', url: 'https://groupme.com', color: '#00AFF0', bg: '#f0faff', icon: '💬'},
  { name: 'iMessage / SMS', url: null, color: '#34C759', bg: '#f0fdf4', icon: '📱',},
  { name: 'Email', url: 'mailto:', color: '#f59e0b', bg: '#fffbeb', icon: '✉️', description: 'Formal communications and newsletters' },
]
    ...
    <div className="page">
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4,1fr)', gap: '1rem', marginBottom: '1.75rem' }}>
        {[
          ...
          { label: 'Upcoming Events', value: upcomingEvents.length, color: '#3b82f6' },
        ].map(s => (
                ...
                <div key={task.task_key} onClick={() => toggleTask(task.task_key)} className="card"
                  ...
                  <div style={{ flex: 1 }}>
                    ...
                    <div style={{ display: 'flex', gap: 8, marginTop: 3 }}>
                      ...
                      {task.due_offset_days != null && <span style={{ fontSize: '0.71rem', color: 'var(--gray-400)' }}>{task.due_offset_days}d before</span>}
                    </div>
                  </div>
                </div>
              ))}
        ...
        <div className="fade-in">
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: '1rem', marginBottom: '1.75rem' }}>
            {SOCIAL_PLATFORMS.map(platform => (
              <div key={platform.name} className="card card-hover" style={{ background: platform.bg, border: `1px solid ${platform.color}25` }}>
                ...
                <p style={{ fontSize: '0.78rem', color: 'var(--gray-500)', lineHeight: 1.5 }}>{platform.description}</p>
              </div>
            ))}
          </div>
          ...
          <div className="card" style={{ marginBottom: '1rem' }}>
            ...
            {socialPosts.length === 0 ? (
              <p style={{ fontSize: '0.84rem', color: 'var(--gray-400)', textAlign: 'center', padding: '1.5rem' }}>No posts logged yet</p>
            ) : (
              <div style={{ display: 'flex', flexDirection: 'column', gap: '0.5rem' }}>
                {socialPosts.map(post => {
                  const plat = SOCIAL_PLATFORMS.find(p => p.name === post.platform)
                  return (
                    <div key={post.id} style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '0.6rem 0.75rem', background: plat?.bg || 'var(--gray-50)', borderRadius: 8, border: `1px solid ${plat?.color || 'var(--border)'}20` }}>
                      <span style={{ fontSize: 18 }}>{plat?.icon || '📱'}</span>
                      <div style={{ flex: 1 }}>
                        <span style={{ fontWeight: 600, fontSize: '0.84rem', color: plat?.color }}>{post.platform}</span>
                        <span style={{ fontSize: '0.82rem', color: 'var(--gray-700)', marginLeft: 8 }}>{post.event}</span>
                        {post.notes && <span style={{ fontSize: '0.78rem', color: 'var(--gray-400)', marginLeft: 8 }}>— {post.notes}</span>}
                      </div>
      ...
      {tab === 'spotlight' && (
        <div className="fade-in">
          <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: '1.25rem', padding: '0.6rem 0.9rem', background: '#fffbeb', borderRadius: 8, border: '1px solid #fde68a' }}>
            <span>⭐</span>
            <span style={{ fontSize: '0.8rem', color: '#92660a', fontWeight: 600 }}>Post one Brother Spotlight per month — boosts morale and promotes ALM on campus.</span>
          </div>
